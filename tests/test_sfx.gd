...
func test_pcm_byte_count_matches_duration() -> void:
	...
	var pcm := SfxSynth.synth_pcm(440.0, 0.1, SfxSynth.Wave.SINE, 0.5, 6.0, RATE)
	assert_eq(float(pcm.size()), float(int(0.1 * RATE) * 2), "bytes = samples * 2")


func test_pcm_respects_sample_rate() -> void:
	var a := SfxSynth.synth_pcm(440.0, 0.05, SfxSynth.Wave.SINE, 0.5, 6.0, 22050)
	var b := SfxSynth.synth_pcm(440.0, 0.05, SfxSynth.Wave.SINE, 0.5, 6.0, 11025)
	assert_eq(float(a.size()), float(b.size()) * 2.0, "halving the rate halves the bytes")


func test_pcm_never_clips() -> void:
	for wave in [SfxSynth.Wave.SINE, SfxSynth.Wave.SQUARE, SfxSynth.Wave.SAW, SfxSynth.Wave.NOISE]:
		var pcm := SfxSynth.synth_pcm(440.0, 0.05, wave, 1.0, 0.0, RATE)
...
func test_envelope_decays() -> void:
	var pcm := SfxSynth.synth_pcm(440.0, 0.2, SfxSynth.Wave.SINE, 0.8, 20.0, RATE)
...
func test_zero_decay_sustains() -> void:
	var pcm := SfxSynth.synth_pcm(440.0, 0.2, SfxSynth.Wave.SINE, 0.8, 0.0, RATE)
	var head := _peak(pcm, 0, 200)
	var tail := _peak(pcm, pcm.size() - 400, pcm.size())
...
func test_volume_scales_amplitude() -> void:
	var quiet := _peak_all(SfxSynth.synth_pcm(440.0, 0.05, SfxSynth.Wave.SINE, 0.2, 0.0, RATE))
	var loud := _peak_all(SfxSynth.synth_pcm(440.0, 0.05, SfxSynth.Wave.SINE, 0.8, 0.0, RATE))
	assert_gt(loud, quiet * 3.0, "four times the volume is roughly four times the peak")


func test_invalid_duration_is_empty() -> void:
	assert_eq(float(SfxSynth.synth_pcm(440.0, 0.0, SfxSynth.Wave.SINE, 0.5, 6.0, RATE).size()), 0.0,
		"zero duration makes no samples")
	assert_eq(float(SfxSynth.synth_pcm(440.0, -1.0, SfxSynth.Wave.SINE, 0.5, 6.0, RATE).size()), 0.0,
		"negative duration is safe")


func test_bake_produces_a_playable_stream() -> void:
	var stream := SfxSynth.bake(440.0, 0.1, SfxSynth.Wave.SINE, 0.5, 6.0)
	...
	assert_eq(float(stream.mix_rate), float(SfxSynth.SAMPLE_RATE), "declared mix rate")
...
func test_bake_rejects_empty_input() -> void:
	assert_true(SfxSynth.bake(440.0, 0.0, SfxSynth.Wave.SINE, 0.5, 6.0) == null,
			"no samples, no stream")


func test_every_recipe_is_complete_and_sane() -> void:
	# A typo in this table would otherwise only surface as silence at runtime.
	for key in SfxSynth.RECIPES:
		var r: Dictionary = SfxSynth.RECIPES[key]
		...
		assert_true(float(r.get("wave", -1)) >= 0.0
			and float(r.get("wave", 99)) <= float(SfxSynth.Wave.NOISE),
...
func test_every_recipe_actually_synthesises() -> void:
	for key in SfxSynth.RECIPES:
		var r: Dictionary = SfxSynth.RECIPES[key]
		var pcm := SfxSynth.synth_pcm(float(r["freq"]), float(r["dur"]), int(r["wave"]),
			float(r["vol"]), float(r["decay"]), SfxSynth.SAMPLE_RATE)
...
func test_recipe_lookup() -> void:
	assert_true(SfxSynth.has_recipe("shot"), "a documented sound exists")
	assert_true(SfxSynth.has_recipe("click"), "menu sounds exist")
	assert_false(SfxSynth.has_recipe("not_a_sound"), "unknown sounds report false")
...
func test_noise_is_deterministic() -> void:
	...
	var a := SfxSynth.synth_pcm(0.0, 0.02, SfxSynth.Wave.NOISE, 0.5, 0.0, RATE)
	var b := SfxSynth.synth_pcm(0.0, 0.02, SfxSynth.Wave.NOISE, 0.5, 0.0, RATE)
	assert_eq(a, b, "same parameters, identical bytes")


func test_noise_stays_in_range() -> void:
	for i in range(64):
		assert_ge(SfxSynth._noise(i), -1.0, "noise floor")
		assert_le(SfxSynth._noise(i), 1.0, "noise ceiling")
