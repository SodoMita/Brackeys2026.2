...
const DIM := Color(0.05, 0.03, 0.01, 0.75)
...
func open_pause() -> void:
	...
	visible = true
	_set_touch_mouse(true)
	if is_inside_tree() and btn_resume:
		btn_resume.grab_focus()


func open_settings(from: Control = null) -> void:
	settings_from = from if from != null else (pause_panel if pause_panel.visible else main_panel)
	settings_open = true
	if settings_from:
		settings_from.visible = false
...
func _build_pause() -> void:
	...
	box.add_child(opts)
	btn_restart = _button("RETRY MISSION")
	btn_restart.pressed.connect(func(): restart_pressed.emit())
	box.add_child(btn_restart)
	btn_menu = _button("QUIT TO MENU")
	btn_menu.pressed.connect(func(): quit_to_menu_pressed.emit())
...
func _close_settings() -> void:
	...
	if settings_from:
		...
		if settings_from == main_panel and is_inside_tree() and btn_start:
			btn_start.grab_focus()
		elif settings_from == pause_panel and is_inside_tree() and btn_resume:
			btn_resume.grab_focus()
